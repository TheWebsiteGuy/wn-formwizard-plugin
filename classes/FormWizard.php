...
use TheWebsiteGuy\FormWizard\Classes\FilePond\FilePond;
use TheWebsiteGuy\FormWizard\Models\Record;
use TheWebsiteGuy\FormWizard\Models\Settings;
use Winter\Storm\Support\Facades\Validator;

abstract class FormWizard extends ComponentBase
{

    use \TheWebsiteGuy\FormWizard\Classes\Traits\PostData;
    use \TheWebsiteGuy\FormWizard\Classes\Traits\ReCaptcha;
    use \TheWebsiteGuy\FormWizard\Classes\Traits\RequestValidation;
    use \TheWebsiteGuy\FormWizard\Classes\Traits\SendMails;
    use \TheWebsiteGuy\FormWizard\Classes\Traits\SharedProperties;
    ...
    public function onRun()
    {
        ...
        if ($this->isReCaptchaMisconfigured()) {
            $this->page['recaptcha_warn'] = Lang::get('thewebsiteguy.formwizard::lang.components.shared.recaptcha_warn');
        }

        if ($this->property('inline_errors') == 'display') {
            $this->addJs('/plugins/thewebsiteguy/formwizard/assets/js/inline-errors.js');
        }
    }
    ...
    public function onFormSubmit()
    {
        ...
        Event::fire('thewebsiteguy.formwizard.beforeSaveRecord', [&$post, $this]);
        ...
        if (!$this->property('skip_database')) {
            ...
            // attach files after save so the record has a valid ID
        ...
        Event::fire('thewebsiteguy.formwizard.afterSaveRecord', [&$post, $this, $record]);
    ...
    public function getSharedPartial($fileName)
    {
        $path = plugins_path('thewebsiteguy/formwizard/classes/partials/' . $fileName);
        if (\File::exists($path)) {
