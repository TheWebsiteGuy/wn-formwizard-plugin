...
use TheWebsiteGuy\FormWizard\Classes\BackendHelpers as BH;
...
class Notification implements Mailable
{
    ...
    {
        $template = $this->properties['mail_template'] ?? '';
        if (is_array($template)) {
            $template = '';
        }

        return !empty($template) && MailTemplate::findOrMakeTemplate($template) ?
            $template :
            'thewebsiteguy.formwizard::mail.notification';
    }
